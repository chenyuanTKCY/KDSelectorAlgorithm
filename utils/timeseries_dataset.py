...
class TimeseriesDataset(Dataset):
	def __init__(self, data_path, fnames, text_data_path=None, verbose=True):
		self.data_path = data_path
		self.fnames = fnames
		self.labels = []
		self.samples = []
		self.text_data = []  # List to hold text data
		self.soft_labels = []
		self.indexes = []
		...
		text_data_path = 'data/MSAD_Text/description/TSB_128'
		...
		for fname in tqdm(self.fnames, disable=not verbose, desc="Loading dataset"):
			...
			self.samples.append(data.iloc[:, 13:].values)
			self.soft_labels.append(data.iloc[:, 1:13].values)
			# self.samples.append(data.iloc[:, 1:].to_numpy())
			# self.text_data.extend(data.iloc[:, 1].values.tolist())
			# Load text data if text_data_path is provided
			if text_data_path:
				text_file = os.path.join(text_data_path, fname)

				df = pd.read_csv(text_file)

				texts = df.iloc[:, 1].dropna().apply(str).tolist()
		...
		# Load text data if text_data_path is provided
			# if text_data_path:
				...
				# self.text_data.extend(lines)
				# print(f"Loaded text for {fname}: {lines}")

		# Concatenate samples and labels
		self.labels = np.asarray(self.labels)
		self.samples = np.concatenate(self.samples, axis=0)
		self.soft_labels = np.concatenate(self.soft_labels, axis=0)
	...
	def __getitem__(self, idx):
		...
		if self.text_data:
			...
			return sample, label, soft_label, text
		return sample, label, soft_label, None
