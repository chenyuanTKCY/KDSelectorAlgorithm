...
def create_tmp_dataset(
	...
	ts_list, labels ,soft_labels = split_and_compute_labels(x, metrics_data, window_size)
	...
	'''
	# Save new dataset
	for ts, label, soft_label, fname in tqdm(zip(ts_list, labels, soft_labels, fnames), total=len(ts_list),
											 desc='Save dataset'):
		fname_split = fname.split('/')
		dataset_name = fname_split[-2]
		ts_name = fname_split[-1]
		...
		# Add soft labels to data
		data = np.concatenate((label[:, np.newaxis], soft_label, ts), axis=1)
		col_names = ['label'] + ["soft_label_{}".format(i) for i in range(soft_label.shape[1])] + ["val_{}".format(i) for i in range(window_size)]
		df = pd.DataFrame(data, index=new_names, columns=col_names)
		df.to_csv(os.path.join(save_dir, name, dataset_name, ts_name + '.csv'))
...
def split_and_compute_labels(x, metrics_data, window_size):
	...
	'''
	for ts, scores in tqdm(zip(x, metrics_data.values), total=len(x), desc="Create dataset"):
		...
		# labels.append(np.ones(len(ts_split)) * detector_names.index(scores.idxmax()))
		labels.append(np.ones(len(ts_split)) * detector_names.index(np.argmax(scores)))

		soft_labels.append(np.tile(scores, (len(ts_split), 1)))  # Replicate scores for each window
	'''
	for ts, metric_label, scores in tqdm(zip(x, metrics_data.idxmax(axis=1), metrics_data.values), total=len(x), desc="Create dataset"):
		# Z-normalization (windows with a single value go to 0)
		ts = z_normalization(ts, decimals=7)
...
def z_normalization(ts, decimals=5):
    # 转换为均值为0、标准差为1的分布
    # 在机器学习中，尤其是使用梯度下降法优化模型时，如果输入数据的尺度差异较大，优化过程可能会变得非常缓慢，甚至难以收敛。
    # 因此，通常需要对数据进行标准化（Standardization）或归一化（Normalization）处理。
